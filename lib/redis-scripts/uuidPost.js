module.exports = ["local c=redis.call",
  ...
  // The length of our list after rpush, if pushing message
  "local l=0",
  // If we're a matched connection
  "if m then",
    // See if this is the first message to a started connection
    "local s=c('pttl','start-listless/'..m)",
    // If it is
    "if s>-1 then",
      // Create our match's list
      "l=c('rpush','message-list/'..m,ARGV[2])",
      // Copy the PTTL of the listless get
      "c('pexpire','message-list/'..m,s)",
      "c('del','start-listless/'..m)",
    "else",
  ...
  "elseif o then",
    ...
    // Hook our new record up to theirs
    "c('psetex','match/'..ARGV[4],t,o)",
