  ...
  function offerPost(path, req, res, next) {
    ...
    db.get('get/path/' + path, function(err, offer){
      ...
      // If this path is empty
        ...
        // Set additional path for TTL refresh on get
        .set('get/id/' + privateId + '/offerat', 'location/path/' + path,
          'PX', itemTTL)
        ...
        // Set initial value
        .set('get/path/' + path, req.body, 'PX', itemTTL)
  ...
  app.get(uuidRouteRegex, function getUuid(req, res, next) {
    ...
    .exec(function (err,results) {
      ...
      if (body || waiting) {
        ...
        // Refresh the location's TTL if we're waiting, too
        if (waiting) transaction.pexpire(waiting, itemTTL);
